...
ssize_t android_show_functions(struct device *dev,
					...
					char *buf)
{
  ...
  list_for_each_entry(f, &android_config_driver.functions, list) {
    if (!f->hidden)
	buff += sprintf(buff, "%s,", (!strcmp(f->name, "f_mass_storage") ? "mass_storage" : f->name));
  }
  if (buff != buf)
    *(buff-1) = '\n';
  return buff - buf;	
...
ssize_t android_store_functions(
	...
	while (b)
	  {
		...
		else if (!strcmp(name, "adb"))
		  func += (1 << USB_FUNCTION_ADB);
		else if (!strcmp(name, "rndis"))
		  func += (1 << USB_FUNCTION_RNDIS);
	      }
	  }
	android_switch_function(func);
	return size;
}
...
int android_switch_function(unsigned func)
{
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {
		...
		if ((func & (1 << USB_FUNCTION_UMS)) &&
			!strcmp(f->name, "f_mass_storage"))
			f->hidden = 0;
#else
		/* send composite uevent when ums state changed
		 */
		if (!strcmp(f->name, "f_mass_storage")) {
