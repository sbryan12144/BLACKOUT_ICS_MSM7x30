...
extern ssize_t android_show_functions(struct device *dev,
					struct device_attribute *attr,
                                             char *buf);
extern ssize_t android_store_functions(
...
static DEVICE_ATTR(functions, S_IRUGO | S_IWUSR, android_show_functions, android_store_functions);
...
composite_unbind(struct usb_gadget *gadget)
{
	...
	device_remove_file(&gadget->dev, &dev_attr_functions);
	composite = NULL;
}
...
static int composite_bind(struct usb_gadget *gadget)
{
	...
	status = device_create_file(&gadget->dev, &dev_attr_functions);
	if (status)
		goto fail;
...
int usb_composite_register(struct usb_composite_driver *driver)
{
	...
	driver->class = class_create(THIS_MODULE, "android_usb");
